...
original_dist = norm(loc = mean, scale = sigma)

gauss_data = original_dist.rvs(N)
average = np.mean(gauss_data)
average_err = sigma/np.sqrt(N)
...
Lsamples = np.array([norm.pdf(mu_proposed, loc = s, scale = sigma) for s in gauss_data]) 
